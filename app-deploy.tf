resource "null_resource" "app-deploy" {
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      "ansible-pull -U https://github.com/ashwinreddy9966/ansible roboshop-pull.yml -e ENV=${var.ENV} -e APP_VERSION=${var.APP_VERSION} -e COMPONENT=${var.COMPONENT} -e MONGODB_ENDPOING=${data.terraform_remote_state.outputs.}"
    ]
  }
}